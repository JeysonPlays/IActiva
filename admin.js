...
function showMainScreen(user) {
  ...
  // Restaurar preferencia de modo rendimiento
  try {
    const perfPref = localStorage.getItem("perf-lite-enabled");
    if (perfPref === "true") {
      document.body.classList.add("perf-lite");
      if (perfModeToggle) {
        perfModeToggle.textContent = "🎨 Desactivar Modo Rendimiento";
        perfModeToggle.setAttribute("aria-pressed", "true");
      }
    }
  } catch (e) {
    console.warn("No se pudo leer preferencia de rendimiento", e);
  }
  if (!isMainScreenShown) {
    setupListeners();
...
// Toggle de modo rendimiento (reduce animaciones / blur costoso)
if (perfModeToggle) {
  perfModeToggle.addEventListener("click", () => {
    const enabled = document.body.classList.toggle("perf-lite");
    try {
      localStorage.setItem("perf-lite-enabled", enabled ? "true" : "false");
    } catch {}
    perfModeToggle.textContent = enabled
      ? "🎨 Desactivar Modo Rendimiento"
      : "⚡ Activar Modo Rendimiento";
    perfModeToggle.setAttribute("aria-pressed", enabled ? "true" : "false");
  });
}

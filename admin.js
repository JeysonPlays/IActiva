...
function renderAdminSections(docs) {
  ...
  // Asegurar limpieza para temas que se quedaron sin secciones
  // (no aparecen en byTema cuando no hay docs)
  temaNodeCache.forEach((cached, temaId) => {
    if (!byTema.has(temaId)) {
      const emptyContainer = document.getElementById(`sections-list-${temaId}`);

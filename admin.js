...
function setupListeners() {
  ...
  unsubscribeTemas = onSnapshot(temasCollection, (snapshot) => {
    ...
    // Re-construir la lista completa de temas
    temasList.innerHTML = "";
    snapshot.docs.forEach((temaDoc) => {
      ...
      // Listeners por elemento creado
      const editBtn = temaDiv.querySelector(".edit-tema-btn");
      ...
      const deleteBtn = temaDiv.querySelector(".delete-tema-btn");
      ...
      const createSectionBtn = temaDiv.querySelector(".create-section-button");
    ...
    // Asegurar que las secciones se muestren con los contenedores ya presentes
    if (latestSectionsDocs.length) {
      renderAdminSections(latestSectionsDocs);
    }
  });
...
// Nota: se eliminó attachTemaButtonListeners en favor de listeners por elemento creado
...
function renderAdminSections(docs) {
  ...
  docs.forEach((sectionDoc) => {
    ...
    const sectionsList = document.getElementById(`sections-list-${temaId}`);
    ...
    if (temaId && sectionsList) {
      ...
      div.innerHTML = `
          <div class="md:w-1/3 section-card-img-wrap">
